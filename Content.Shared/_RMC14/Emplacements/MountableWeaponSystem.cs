...
public sealed class MountableWeaponSystem : EntitySystem
{
    ...
    private void OnAttemptShoot(Entity<MountableWeaponComponent> ent, ref AttemptShootEvent args)
    {
        ...
        if (!TryComp(ent, out GunComponent? gun) || gun.ShootCoordinates is not { } toCoordinates)
            return;
        ...
        var aimedLocation = _transform.ToWorldPosition(toCoordinates);
        ...
        if (Math.Abs(normalizedDirection) > ent.Comp.ShootArc / 2f)
        {
            ...
            // Rotate the mount to the aimed location if its rotation is not locked
            if (TryComp(mountEntity, out WeaponMountComponent? mount) &&
                mount.CanRotateWithoutTool)
            {
        ...
        if (!TryComp(args.User, out HandsComponent? hands) || hands.CountFreeHands() < ent.Comp.RequiredFreeHands)
        {
            args.Cancelled = true;
